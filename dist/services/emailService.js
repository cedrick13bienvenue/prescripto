...
class EmailService {
     ...
     */
    static async sendPrescriptionEmail(data) {
        try {
     ...
     */
    static async sendPasswordResetEmail(userEmail, resetToken, userName) {
        try {
            const transporter = await this.getTransporter();
     ...
     */
    static generatePrescriptionEmailHTML(data) {
     ...
     */
    static generatePrescriptionEmailText(data) {
        const medicinesList = data.medicines.map(medicine => `- ${medicine.name}: ${medicine.dosage}, ${medicine.frequency}, Qty: ${medicine.quantity}${medicine.instructions ? ` (${medicine.instructions})` : ''}`).join('\n');
        return `
...
This is an automated message from MedConnect. Please do not reply to this email.
     ...
     */
    static generateOTPEmailHTML(data) {
     ...
     */
    static generateOTPEmailText(data) {
        const expiryTime = data.expiresAt.toLocaleString();
        return `
...
This is an automated security message from MedConnect. Please do not reply to this email.
     ...
     */
    static getRoleSpecificWelcomeMessage(role) {
        switch (role) {
            case models_1.UserRole.PATIENT:
                return `
        ...
        `;
            case models_1.UserRole.PHARMACIST:
                return `
          <ul style="margin: 0; padding-left: 20px;">
