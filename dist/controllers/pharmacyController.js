"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.PharmacyController = void 0;
const pharmacyService_1 = require("../services/pharmacyService");
class PharmacyController {
    /**
     * Scan QR code and get prescription details
     * POST /api/v1/pharmacy/scan
     */
    static async scanQRCode(req, res) {
        try {
            const { qrHash } = req.body;
            if (!qrHash) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: 'QR hash is required',
                        statusCode: 400,
                    },
                });
            }
            if (!req.user) {
                return res.status(401).json({
                    success: false,
                    error: {
                        message: 'Authentication required',
                        statusCode: 401,
                    },
                });
            }
            const result = await pharmacyService_1.PharmacyService.scanQRCode(qrHash, req.user.id);
            if (!result.isValid) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: result.message,
                        statusCode: 400,
                    },
                });
            }
            res.status(200).json({
                success: true,
                message: result.message,
                data: {
                    prescription: result.prescription,
                    isValid: result.isValid,
                    canDispense: result.canDispense,
                },
            });
        }
        catch (error) {
            console.error('QR code scan error:', error);
            res.status(500).json({
                success: false,
                error: {
                    message: error.message || 'Failed to scan QR code',
                    statusCode: 500,
                },
            });
        }
    }
    /**
     * Validate prescription (pharmacist reviews and approves)
     * POST /api/v1/pharmacy/validate/:prescriptionId
     */
    static async validatePrescription(req, res) {
        try {
            const { prescriptionId } = req.params;
            const { notes } = req.body;
            if (!prescriptionId) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: 'Prescription ID is required',
                        statusCode: 400,
                    },
                });
            }
            if (!req.user) {
                return res.status(401).json({
                    success: false,
                    error: {
                        message: 'Authentication required',
                        statusCode: 401,
                    },
                });
            }
            const result = await pharmacyService_1.PharmacyService.validatePrescription(prescriptionId, req.user.id, notes);
            if (!result.success) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: result.message,
                        statusCode: 400,
                    },
                });
            }
            res.status(200).json({
                success: true,
                message: result.message,
                data: result.prescription,
            });
        }
        catch (error) {
            console.error('Prescription validation error:', error);
            res.status(500).json({
                success: false,
                error: {
                    message: error.message || 'Failed to validate prescription',
                    statusCode: 500,
                },
            });
        }
    }
    /**
     * Dispense prescription (pharmacist gives medicine to patient)
     * POST /api/v1/pharmacy/dispense/:prescriptionId
     */
    static async dispensePrescription(req, res) {
        try {
            const { prescriptionId } = req.params;
            const { notes } = req.body;
            if (!prescriptionId) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: 'Prescription ID is required',
                        statusCode: 400,
                    },
                });
            }
            if (!req.user) {
                return res.status(401).json({
                    success: false,
                    error: {
                        message: 'Authentication required',
                        statusCode: 401,
                    },
                });
            }
            const result = await pharmacyService_1.PharmacyService.dispensePrescription(prescriptionId, req.user.id, notes);
            if (!result.success) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: result.message,
                        statusCode: 400,
                    },
                });
            }
            res.status(200).json({
                success: true,
                message: result.message,
                data: result.prescription,
            });
        }
        catch (error) {
            console.error('Prescription dispensing error:', error);
            res.status(500).json({
                success: false,
                error: {
                    message: error.message || 'Failed to dispense prescription',
                    statusCode: 500,
                },
            });
        }
    }
    /**
     * Reject prescription (pharmacist rejects due to issues)
     * POST /api/v1/pharmacy/reject/:prescriptionId
     */
    static async rejectPrescription(req, res) {
        try {
            const { prescriptionId } = req.params;
            const { reason } = req.body;
            if (!prescriptionId) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: 'Prescription ID is required',
                        statusCode: 400,
                    },
                });
            }
            if (!reason) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: 'Rejection reason is required',
                        statusCode: 400,
                    },
                });
            }
            if (!req.user) {
                return res.status(401).json({
                    success: false,
                    error: {
                        message: 'Authentication required',
                        statusCode: 401,
                    },
                });
            }
            const result = await pharmacyService_1.PharmacyService.rejectPrescription(prescriptionId, req.user.id, reason);
            if (!result.success) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: result.message,
                        statusCode: 400,
                    },
                });
            }
            res.status(200).json({
                success: true,
                message: result.message,
                data: result.prescription,
            });
        }
        catch (error) {
            console.error('Prescription rejection error:', error);
            res.status(500).json({
                success: false,
                error: {
                    message: error.message || 'Failed to reject prescription',
                    statusCode: 500,
                },
            });
        }
    }
    /**
     * Get prescription logs
     * GET /api/v1/pharmacy/logs/:prescriptionId
     */
    static async getPrescriptionLogs(req, res) {
        try {
            const { prescriptionId } = req.params;
            if (!prescriptionId) {
                return res.status(400).json({
                    success: false,
                    error: {
                        message: 'Prescription ID is required',
                        statusCode: 400,
                    },
                });
            }
            const logs = await pharmacyService_1.PharmacyService.getPrescriptionLogs(prescriptionId);
            res.status(200).json({
                success: true,
                data: { logs },
            });
        }
        catch (error) {
            console.error('Error fetching prescription logs:', error);
            res.status(500).json({
                success: false,
                error: {
                    message: error.message || 'Failed to fetch prescription logs',
                    statusCode: 500,
                },
            });
        }
    }
    /**
     * Get pharmacist's dispensing history
     * GET /api/v1/pharmacy/history
     */
    static async getPharmacistHistory(req, res) {
        try {
            const { page = 1, limit = 10 } = req.query;
            const pageNum = parseInt(page, 10);
            const limitNum = parseInt(limit, 10);
            if (!req.user) {
                return res.status(401).json({
                    success: false,
                    error: {
                        message: 'Authentication required',
                        statusCode: 401,
                    },
                });
            }
            const result = await pharmacyService_1.PharmacyService.getPharmacistHistory(req.user.id, pageNum, limitNum);
            res.status(200).json({
                success: true,
                data: result,
            });
        }
        catch (error) {
            console.error('Error fetching pharmacist history:', error);
            res.status(500).json({
                success: false,
                error: {
                    message: error.message || 'Failed to fetch pharmacist history',
                    statusCode: 500,
                },
            });
        }
    }
    // Check QR code scan status
    static async checkScanStatus(req, res) {
        try {
            const { qrHash } = req.params;
            if (!qrHash) {
                res.status(400).json({
                    success: false,
                    message: "QR hash is required",
                });
                return;
            }
            const result = await pharmacyService_1.PharmacyService.checkQRScanStatus(qrHash);
            if (!result) {
                res.status(404).json({
                    success: false,
                    message: "QR code not found",
                });
                return;
            }
            res.status(200).json({
                success: true,
                message: "QR code scan status retrieved",
                data: result,
            });
        }
        catch (error) {
            console.error("Error checking scan status:", error);
            res.status(500).json({
                success: false,
                message: "Internal server error",
                error: error.message,
            });
        }
    }
    /**
     * Get dispensing history for a prescription
     */
    static async getDispensingHistory(req, res) {
        try {
            const { prescriptionId } = req.params;
            const history = await pharmacyService_1.PharmacyService.getDispensingHistory(prescriptionId);
            res.status(200).json({
                success: true,
                message: "Dispensing history retrieved successfully",
                data: history,
            });
        }
        catch (error) {
            console.error("Error getting dispensing history:", error);
            res.status(500).json({
                success: false,
                message: "Internal server error",
                error: error.message,
            });
        }
    }
    /**
     * Get dispensing summary for a prescription
     */
    static async getDispensingSummary(req, res) {
        try {
            const { prescriptionId } = req.params;
            const summary = await pharmacyService_1.PharmacyService.getDispensingSummary(prescriptionId);
            res.status(200).json({
                success: true,
                message: "Dispensing summary retrieved successfully",
                data: summary,
            });
        }
        catch (error) {
            console.error("Error getting dispensing summary:", error);
            res.status(500).json({
                success: false,
                message: "Internal server error",
                error: error.message,
            });
        }
    }
}
exports.PharmacyController = PharmacyController;
